...
function App() {
  ...
  const contractAddress = ""; // Paste the address recieved from Remix IDE here
  const [totalRegistered, setTotalRegistered] = useState(0);

  
  //Fancy Cursor
  // Create two custom cursor elements
  ...
  // Update its position based on mouse movement
  document.addEventListener('mousemove', (e) => {
    cursor.style.left = `${e.clientX}px`;
    cursor.style.top = `${e.clientY}px`;
  });
    ...
    const checkRegistration = async () => {
      ...
      try {
        ...
        if (registered) {
          ...
          await fetchTotalRegisteredPeople();//Get total number of registered users
        }
      } catch (error) {
  ...
  const changeName = async () => {
    ...
    try {
      const tx = await contract.updateName(newName.trim()); // Make sure this matches your contract method
      await tx.wait();
  ...
  const fetchTotalRegisteredPeople = async () => {
    if (!contract) return;
    try {
      const count = await contract.getTotalRegisteredPeople(); // or whatever method your smart contract uses
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        : (
          <div>
            <h2 style={{ color: '#ffc621' }}>Welcome, {name}</h2>
            <p style={{ color: '#fe7502',fontSize:'20px'}}>Account: {account}</p>
            ...
            <div className="change-name">
              ...
              style={{
                ...
                textAlign: 'center' // This centers the placeholder and input text horizontally
              }}
            />
              <p>Total Registered Users : {totalRegistered}</p>
              <button style={{}}onClick={changeName}>Change Name</button>
            </div>
          )}
